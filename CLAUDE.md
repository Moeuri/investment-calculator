...
### v2.1 規劃中（功能/UI，已定案 2026-06-30）

高優先、低風險，集中於定期定額分頁與計算年限。三項確切範圍如下：

**① 定期定額報酬率「自訂值」**
- 只動 `src/tabs/DCATab.jsx`，不碰 `utils.js`。
- 六檔預設按鈕下方加一條「自訂年化」`Slider`（建議 4%–20%，step 0.5%）。
- 拖動自訂 → `dr` 設為任意值，此時六檔皆不高亮（`activePreset` 為 undefined 時的既有邏輯已可處理）；點任一預設 → snap 回該值。

**② 觀察年限可調整（本版最大項，因 241 個月寫死）**
- 範圍 **10–40 年，預設 20**（決策 A）。超過 40 年複利數字與扇形 σ=波動×√t 會大到失去參考意義，故設上限。
- 新增全域狀態 `years`（觀察年限，單位：年）至 `App.jsx` INIT，預設 20；UI 放在定期定額分頁（`Slider`，step 5 或可輸入）。
- **扣款期數 `per` 上限自動連動 = `years × 12`**（不能投得比觀察期久）；現行 `per` 上限 72 月一併放寬。
- **必須參數化的函數（utils.js）**：
  - `buildCrashN(lumpSum, amt, per, annR, crashes, months=240)` → 傳遞 months
  - `buildCrashClean(..., months)` → `Float64Array(241)`改`months+1`、`mo <= 240`改`mo <= months`
  - `calcFan(centralVals, fanStartMo, annVol, months=240)` → 同上把 241/240 換掉
  - `buildNorm` 已支援 `months` 參數，免改
- **分頁改動**：定期定額、崩盤兩個圖表重的分頁，把 `Array.from({length: 20})` 與所有寫死的 `[240]` 索引（如 `norm9816[240]`、`cash0050[240]`）改為以 `years` / `years*12` 計算。
- **本版生效範圍（決策 B）**：僅「定期定額 + 崩盤」兩分頁連動 `years`；通膨、退休維持各自既有 20 年邏輯不變。

**③ 圖表「實質購買力（扣通膨）」切換**
- 本版範圍（決策 C）：**僅定期定額分頁**先做。
- 做法：圖表上方加 toggle（分頁 local state 即可，免新全域狀態），開啟時把每條序列值與摘要卡數字一併除以 `(1+infl)^(月索引/12)`（沿用全域 `infl`）。注意「圖表」與「摘要 Card」要一起換算，避免兩者口徑不一致。

### 既定方向（已承諾，預計 v2.2–v2.3 內處理）

- **退休提領分頁觀察年限與全域 `years` 連動**（決策 B 延伸）：本版先不動，但列為既定方向。退休分頁本身已有 `retireAfter` / `drawYears` 兩段時間軸，連動時需釐清「全域觀察年限」與「退休後提領年數」的關係，避免語意衝突——屬需要設計的工作，非單純改數字。
- **實質購買力切換推廣至其他分頁**（決策 C 延伸）：未來可覆蓋
  - 崩盤模擬：中央線 + 扇形上下緣同步除以通膨平減因子（注意扇形已是名目，換算後區間會收窄）。
  - 退休提領 / 通膨分頁：兩者**本身已在邏輯內使用 `infl`**，加 toggle 時必須避免「重複扣通膨」（double-counting），需逐頁檢視其現有 infl 用法後再決定是否套用。
  - 預期做法：把「名目→實質」換算抽成 `utils.js` 共用函數 `toReal(series, infl, startYear=0)`，各分頁圖表建構時呼叫，集中口徑。

### 中價值（未來可能性，尚未排程）
- **情境快照 A/B 對比**：以記憶體 state 暫存 2–3 組參數並排比較（不得用 localStorage）
- **手機版 Tab bar 捲動提示**：右側漸層遮罩，提示尚有更多分頁
- **結果匯出**：摘要卡/圖表輸出成圖片或可複製文字（需引 canvas，複雜度略高）

### 大工程（屬模型決策，暫不動）
- 崩盤模擬蒙地卡羅化（會改變所有數字）
- `algoAmonthly` 線性→指數恢復（同上，會動到全部崩盤輸出）
