...
| 儲蓄險 vs 股市 | 優缺點比較 + 「維持儲蓄險 vs 投入大盤」兩線（大盤金額讀自定期定額分頁） | 全域 |
| 關於 | 版本紀錄、版權、免責聲明 | 無 |
...
### v2.7 完成（2026-06-30）— 取代 v2.3 三路徑
- **儲蓄險 vs 股市重新定位**：數字試算由 v2.3 的「維持／解約一次性／解約定期定額」三路徑，改為 **「維持儲蓄險 vs 投入大盤」兩線**。
- **關鍵改動**：大盤投入金額不再由解約金 `netP` 推算，改為**讀「定期定額」分頁的 `lumpSum`／`amt`／`per`**（InsTab 現多讀這三個全域值，比照既有的 `dr`）。大盤線 ＝ `buildNorm(ls, amt, per, r1)`，與定期定額分頁 009816 同一條。
- **理由**：離開儲蓄險的錢只有一部分進大盤（其餘買個股／其他ETF／其他保單／花用），故非等額比較，而是**資本效率**——「只動用解約金的一部分，靠 9% 複利仍追過 2% 整筆儲蓄險」。
- 移除 netP 推算的一次性/定期定額/閒置現金邏輯（未投入的錢不是閒置，是另作他用）。卡片顯示投入金額、佔解約金 `pct`、其餘 `otherFunds`，refLine 標 `mktCross`；大盤線誠實標示僅「投入的部分」。
