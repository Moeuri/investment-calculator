...
### 待修正（已確認，下次處理）
- **通膨購買力分頁：儲蓄險購買力線漏算本金**。`InflTab.jsx` 的「儲蓄險實質累積」目前只累加逐年折現的年領利息（`insAnn/(1+infl)^y`），**未納入本金 `insPrin`**。本金雖保留在保單內，但其實質購買力會逐年變薄（`insPrin/(1+infl)^y`），現況使這條線與「真正的儲蓄險實質總值」始終差一個逐年遞減的本金。
  - 修正方向：儲蓄險實質總值(y) ＝ `insPrin/(1+infl)^y`（仍持有本金的實質值）＋ `Σ insAnn/(1+infl)^k`（累積已領利息實質值）。需同步檢視該分頁「20年總領回實質價值」卡與底部 Note 的口徑，避免本金/利息混淆。
