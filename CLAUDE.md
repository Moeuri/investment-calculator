...
### 待調查（正確性議題，優先級高於圖片匯出）
- **崩盤模擬數字疑點**：製作人於近期測試發現崩盤輸出「可能有問題的結果」。待 v2.4 兩項簡易功能完成後，與製作人對焦具體情境（哪組參數、哪個數字看起來不對）再決定是否需要修模型。可能與下方 `algoAmonthly` 恢復曲線有關，待釐清。

### 待討論（已表達興趣，尚未動手；優先級低於上方崩盤疑點）
- **結果匯出（圖片）**：製作人有興趣，但需先討論「預想輸出的圖長什麼樣」（摘要卡截圖？單張圖表？整頁？）再決定做法與是否引入 `html2canvas`。
  - 低成本替代：純文字摘要複製到剪貼簿（無套件），可一併討論。
