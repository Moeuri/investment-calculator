...
const VERSIONS = [
  ...
  },
  {
    ...
    changes: [
      ...
      '所有滑桿數值改為可點擊直接輸入',
      '各滑桿範圍調整：每月投入1000元起、定期定額6–72個月、退休提領5000元起',
    ],
  },
  {
    ver: 'v1.1',
    ...
    changes: [
      '初始完整版發布，支援六大功能分頁',
      '定期定額、保險vs股市、崩盤模擬（雙崩盤）、通膨購買力、退休提領、高股息ETF',
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--c-green)', marginBottom: 4 }}>
          大盤投資計算器
        </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: 'var(--c-text2)', lineHeight: 1.8 }}>
          ...
          <div style={{ marginTop: 6, color: 'var(--c-text3)' }}>
            個人非商業使用及學習參考不在此限。
          </div>
        </div>
      </div>
      ...
      {VERSIONS.map((v, i) => (
        <div key={v.ver} style={{
          ...
          borderLeft: `2px solid ${i === 0 ? 'var(--c-green)' : 'var(--c-border)'}`,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{
              fontSize: 13, fontWeight: 700,
              color: i === 0 ? 'var(--c-green)' : 'var(--c-text)',
            }}>{v.ver}</span>
            <span style={{ fontSize: 11, color: 'var(--c-text3)' }}>{v.date}</span>
            {i === 0 && (
              <span style={{
                fontSize: 10, padding: '1px 6px', borderRadius: 4,
                background: 'var(--c-green-bg)', color: 'var(--c-green)', fontWeight: 600,
              }}>最新</span>
            )}
          </div>
