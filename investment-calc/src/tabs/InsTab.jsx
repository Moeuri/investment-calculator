...
import { Card, Note, Slider, SectionTitle, InvestChart, Divider } from '../components'
import { fmtM, fmtPA, EXP1 } from '../utils'

export default function InsTab({ state }) {
  const { insPrin, insAnn, insPen, dr } = state
  ...
  return (
    <div>
      ...
      <Slider label="保險本金" min={1000000} max={20000000} step={100000}
        value={insPrin} onChange={v => {}} fmt={v => fmtM(v)} />
      <Slider label="每年領回金額" min={50000} max={500000} step={10000}
        value={insAnn} onChange={v => {}} fmt={v => fmtM(v)} />
      <Slider label="解約費用 %" min={0} max={5} step={0.5}
        value={insPen} onChange={v => {}} fmt={v => v === 0 ? '0%（已過鎖定期）' : v.toFixed(1) + '%'} />

      <Note type="warn" mt={8}>
        此分頁的保險參數需在下方直接調整，與定期定額分頁的保險設定共用同一組數值。
      </Note>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, margin: '12px 0' }}>
        ...
        <Card label="009816超越年份" value={insCross ? `第 ${insCross} 年` : '20年內未超越'} accent={insCross ? 'var(--c-green)' : 'var(--c-red)'} />
      </div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 7, fontSize: 11, color: 'var(--c-text3)' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 12, height: 3, background: '#BA7517', display: 'inline-block' }} />保險總資產</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 12, height: 3, background: '#1D9E75', display: 'inline-block' }} />解約轉 009816</span>
      </div>
      ...
      <Note mt={8}>
        ...
        {insCross
          ...
          : ' 009816 在 20 年內未超越保險總資產，可調高報酬率情境再評估。'}
      </Note>
    </div>
  )
}
